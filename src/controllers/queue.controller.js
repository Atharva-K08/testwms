...
const assignTanker = async (req, res) => {
  const { tankerNumber, driverName, driverMobile } = req.body;

  const updated = await queueService.assignTanker({
    requestId: req.params.id,
    tankerAssignment: { tankerNumber, driverName, driverMobile },
...
const completeRequest = async (req, res) => {
  ...
  sendSuccess(res, { message: 'Request marked as completed.', data: { request: completed } });
};

module.exports = { getQueue, peekNext, assignTanker, completeRequest };
