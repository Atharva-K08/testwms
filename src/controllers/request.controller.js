...
const getMyRequests = async (req, res) => {
  ...
  const result = await requestService.getMemberRequests({
    userId: req.user._id,
    page,
    limit,
  });
...
const getCancelledRequests = async (req, res) => {
  const page = parseInt(req.query.page) || PAGINATION.DEFAULT_PAGE;
  const limit = Math.min(
    parseInt(req.query.limit) || PAGINATION.DEFAULT_LIMIT,
    PAGINATION.MAX_LIMIT,
  ...
  const result = await requestService.getCancelledRequests({ page, limit });

  sendPaginated(res, {
    message: "Cancelled requests fetched.",
    data: result.items,
    page,
    limit,
    total: result.total,
