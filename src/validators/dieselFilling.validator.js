...
const recordDieselFillingValidator = [
  ...
  body('dateTime')
    .optional()
    .isISO8601().withMessage('Date time must be a valid ISO 8601 date.'),

  body('dieselAmount')
    .isFloat({ min: 0 }).withMessage('Diesel amount must be a non-negative number.'),

  body('liters')
    .isFloat({ min: 0 }).withMessage('Liters must be a non-negative number.'),
];

const updateDieselFillingValidator = [
  param('id')
    .isMongoId().withMessage('Invalid diesel filling ID.'),
